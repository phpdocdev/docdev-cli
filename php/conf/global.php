...
class Debug
{
    ...
    public static function var_dump_plain($var, $expLevel, $depth = 0, $done = array())
    {
        ...
        if (is_bool($var)) {
            ...
            if (!empty($var)) {
                $html .= ' <div style="display:inline"><span id="' . $uuid . '-collapsable" style="' . $setStyle . '"><br />[<br />';
                $indent = 4;
                $longest_key = 0;
...
function dbg($defined_vars = [])
{
    if (count($defined_vars) === 0) {
        $defined_vars = [
